...
import fullSignatureMap
import fullDEGeneMap
import detailedSignatureUpload
import detailedGeneUpload
import signmatch_pgr
import enumerateSetOfFilters
import makerangeUI
import makeComparison
import superEventMap
import superSignatureUpload
#import psycopg2
...
if(dattype == "new"):
        ...
        cancerlisting = os.listdir(newdir)
        print("Comparing... ")
elif(dattype == "fullsig"):
        targetdir = newdir
        cancerlisting = os.listdir(newdir)
elif(dattype == "fullDE" or dattype == "superMap"):
        targetdir = newdir
        cancerlisting = os.listdir(newdir)
else:
...
class Cancer:
        ...
        def detailedSignatureMap(self):
                fullSignatureMap.mapAll(self)
                detailedSignatureUpload.makequery(self)

        def listUniqueGenes(self):
                fullDEGeneMap.mapAll(self)
                detailedGeneUpload.makequery(self)
        ...
        def updateSuperMap(self):
                superEventMap.mapAll(self)
...
if(dattype == "superMap"):
        superEventMap.setupFile()
        for i in CANCERS:
                i.updateSuperMap()
        superSignatureUpload.makequery(rootdir)
